...
import { FaFilm, FaStar, FaSearch, FaTh, FaList, FaChevronLeft, FaChevronRight, FaClock, FaCalendarAlt } from 'react-icons/fa';
...
const MoviesPage = () => {
  ...
  const [viewMode, setViewMode] = useState(() => {
    const savedMode = localStorage.getItem('movies_view_mode');
  ...
  const handleViewModeChange = (mode) => {
    setViewMode(mode);
    localStorage.setItem('movies_view_mode', mode);
  };

  const handlePageChange = (page) => {
    setCurrentPage(page);
    localStorage.setItem('movies_current_page', page);
  };
  ...
  useEffect(() => {
    const loadAllMovies = async () => {
      setLoading(true);
      try {
        const categoriesList = ['arabwood', 'hollywood', 'bollywood', 'european', 'asian'];
  ...
  useEffect(() => {
    ...
    handlePageChange(1);
  }, [searchTerm, selectedGenre, selectedYear, selectedCategory, allMovies]);
      ...
      <div className="relative h-[20vh] md:h-[25vh] w-full overflow-hidden">
        ...
        <div className="relative h-full w-full px-3 sm:px-4 md:px-6 lg:px-8 flex flex-col justify-center z-20">
          <div className="flex items-center gap-2 mb-1">
            <FaFilm className="text-red-500 text-xl md:text-2xl" />
            <h1 className="text-xl md:text-2xl font-bold text-white">
              {language === 'ar' ? 'الأفلام' : 'Movies'}
            </h1>
          </div>
          <p className="text-gray-300 text-xs sm:text-sm max-w-2xl">
            {language === 'ar' ? 'استمتع بمشاهدة أحدث وأفضل الأفلام من جميع أنحاء العالم' : 'Enjoy watching the latest and best movies from around the world'}
          </p>
        </div>
      </div>
      ...
      <div className="w-full px-3 sm:px-4 md:px-6 lg:px-8 py-4">
        ...
        {filteredMovies.length === 0 ? (
          <div className="text-center py-12">
            ...
            <p className="text-gray-500 text-lg">{t('movies.noMovies')}</p>
      ...
      <div className="flex gap-3 bg-gray-900/50 rounded-lg p-2 hover:bg-gray-800 transition border border-gray-800 hover:border-red-500/50">
        ...
        <div className="flex-1">
          <div className="flex justify-between items-start">
            ...
            <span className="bg-gray-800 text-gray-400 text-[9px] px-1.5 py-0.5 rounded-full">{getCategoryName(movie.category)}</span>
          </div>
