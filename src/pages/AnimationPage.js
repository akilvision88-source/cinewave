...
const AnimationPage = () => {
  ...
  const showToast = (message, type = 'success') => {
    ...
    toast.className = `fixed bottom-24 left-1/2 transform -translate-x-1/2 z-50 text-white px-4 py-2 rounded-full shadow-lg text-sm flex items-center gap-2 animate-fadeIn ${type === 'success' ? 'bg-green-600' : 'bg-red-600'}`;
  ...
  useEffect(() => {
    if (activeTab === 'movies') {
      let results = [...movies];
      if (searchTerm) {
        results = results.filter(m => m.title.toLowerCase().includes(searchTerm.toLowerCase()) || (m.title_ar && m.title_ar.includes(searchTerm)));
      }
      ...
      let results = [...series];
      if (searchTerm) {
        results = results.filter(s => s.title.toLowerCase().includes(searchTerm.toLowerCase()) || (s.title_ar && s.title_ar.includes(searchTerm)));
      }
      if (selectedGenre !== 'all') {
  ...
  return (
    <div className="min-h-screen bg-black">
      <div className="relative h-[25vh] md:h-[30vh] w-full overflow-hidden">
        ...
        <div className="absolute inset-0 bg-cover bg-center" style={{ backgroundImage: 'url(https://image.tmdb.org/t/p/original/7IiTTgloJzvGI1TAYymCfbfl3vT.jpg)', filter: 'brightness(0.3)' }} />
        <div className="relative h-full w-full px-3 sm:px-4 md:px-6 lg:px-8 flex items-center z-20">
          <div>
            ...
            <p className="text-gray-300 text-sm md:text-base max-w-2xl">استمتع بمشاهدة أجمل أفلام ومسلسلات الرسوم المتحركة والأنيميشن من جميع أنحاء العالم</p>
          </div>
        </div>
      </div>
      ...
      <div className="w-full px-3 sm:px-4 md:px-6 lg:px-8 py-4">
        <div className="flex gap-2 md:gap-3 mb-4 border-b border-gray-800 pb-2">
          <button
            ...
            className={`px-3 md:px-5 py-1.5 md:py-2 rounded-full text-xs md:text-sm font-medium transition-all duration-200 flex items-center gap-1.5 md:gap-2 ${activeTab === 'movies' ? 'bg-red-600 text-white shadow-lg shadow-red-600/30' : 'bg-gray-800 text-gray-400 hover:bg-gray-700 hover:text-white'}`}
          >
          ...
          <button
            ...
            className={`px-3 md:px-5 py-1.5 md:py-2 rounded-full text-xs md:text-sm font-medium transition-all duration-200 flex items-center gap-1.5 md:gap-2 ${activeTab === 'series' ? 'bg-red-600 text-white shadow-lg shadow-red-600/30' : 'bg-gray-800 text-gray-400 hover:bg-gray-700 hover:text-white'}`}
          >
        ...
        <div className="flex flex-col gap-3 mb-4">
          <div className="flex flex-col md:flex-row gap-3">
            ...
            <div className="flex gap-2">
              <select value={selectedGenre} onChange={(e) => setSelectedGenre(e.target.value)} className="bg-gray-800 border border-gray-700 rounded-lg px-2 md:px-3 py-2 text-white text-xs md:text-sm flex-1">
                {(activeTab === 'movies' ? movieGenres : seriesGenres).map(g => (
                  <option key={g} value={g}>{g === 'all' ? t('common.allGenres') : g}</option>
                ))}
              </select>
              <select value={selectedYear} onChange={(e) => setSelectedYear(e.target.value)} className="bg-gray-800 border border-gray-700 rounded-lg px-2 md:px-3 py-2 text-white text-xs md:text-sm">
          ...
          <div className={`flex ${isMobile ? 'justify-center' : 'justify-end'}`}>
            <div className="flex gap-1 bg-gray-800 rounded-lg p-1">
              <button onClick={() => handleViewModeChange('grid')} className={`px-2 md:px-3 py-1 md:py-1.5 rounded-md transition text-xs md:text-sm flex items-center gap-1 ${viewMode === 'grid' ? 'bg-red-600 text-white' : 'text-gray-400 hover:bg-gray-700'}`} title="عرض شبكي">
                <FaTh size={12} /><span className="hidden sm:inline">شبكة</span>
              </button>
              <button onClick={() => handleViewModeChange('list')} className={`px-2 md:px-3 py-1 md:py-1.5 rounded-md transition text-xs md:text-sm flex items-center gap-1 ${viewMode === 'list' ? 'bg-red-600 text-white' : 'text-gray-400 hover:bg-gray-700'}`} title="عرض قائمة">
                <FaList size={12} /><span className="hidden sm:inline">قائمة</span>
              </button>
            </div>
          </div>
        </div>
        ...
        {currentData.length === 0 ? (
          <div className="text-center py-12 bg-gray-900/30 rounded-xl">
            {activeTab === 'movies' ? <FaFilm className="text-gray-700 text-5xl mx-auto mb-4" /> : <FaTv className="text-gray-700 text-5xl mx-auto mb-4" />}
            ...
            <Link to="/admin" className="inline-block mt-4 bg-red-600 text-white px-4 py-2 rounded-lg text-sm hover:bg-red-700 transition">الذهاب للوحة التحكم</Link>
          </div>
        ) : viewMode === 'grid' ? (
        ...
        {totalPages > 1 && (
          <div className="flex justify-center items-center gap-1 mt-8 mb-4">
            <button onClick={goToPrevPage} disabled={currentPage === 1} className={`p-1.5 md:p-2 rounded-lg transition ${currentPage === 1 ? 'bg-gray-800 text-gray-600 cursor-not-allowed' : 'bg-gray-800 text-white hover:bg-red-600'}`}>
              <FaChevronRight size={isMobile ? 12 : 14} />
            </button>
            {getPageNumbers().map(number => (
              <button key={number} onClick={() => goToPage(number)} className={`w-6 h-6 md:w-8 md:h-8 rounded-lg transition text-xs md:text-sm ${currentPage === number ? 'bg-red-600 text-white' : 'bg-gray-800 text-gray-400 hover:bg-gray-700'}`}>{number}</button>
            ))}
            <button onClick={goToNextPage} disabled={currentPage === totalPages} className={`p-1.5 md:p-2 rounded-lg transition ${currentPage === totalPages ? 'bg-gray-800 text-gray-600 cursor-not-allowed' : 'bg-gray-800 text-white hover:bg-red-600'}`}>
    ...
    <div className="block group relative">
      <Link to={`/movie/${movie.id}`} className="block">
        <div className="relative rounded-lg overflow-hidden bg-gray-900">
          ...
          <div className="absolute bottom-1 left-1 bg-purple-600/80 backdrop-blur-sm rounded-full px-1 py-0.5 hidden sm:block">
            <span className="text-white text-[6px] xs:text-[8px]">🎨 رسوم متحركة</span>
          </div>
        </div>
      ...
      <button onClick={(e) => onToggleWatchlist(movie.id, itemType, e)} className={`absolute bottom-14 right-1 p-1 rounded-full transition ${isInWatchlist ? 'bg-purple-600 text-white' : 'bg-gray-800/80 text-gray-400 hover:bg-gray-700'}`} title={isInWatchlist ? 'في قائمتي' : 'إضافة إلى قائمتي'}>
        <FaBookmark size={10} />
      </button>
    </div>
  );
  ...
  return (
    <div className="block group relative">
      <Link to={`/series/${series.id}`} className="block">
        <div className="relative rounded-lg overflow-hidden bg-gray-900">
          <img src={series.poster} alt={series.title} className="w-full aspect-[2/3] object-cover transition-transform duration-300 group-hover:scale-105" loading="lazy" />
          <div className="absolute top-1 left-1 bg-black/70 backdrop-blur-sm rounded-full px-1 py-0.5 flex items-center gap-0.5">
            <FaStar className="text-yellow-400 text-[8px] xs:text-[10px]" />
            <span className="text-white text-[8px] xs:text-[10px] font-semibold">{series.rating || '?'}</span>
          </div>
          {series.seasons && (
            <div className="absolute top-1 right-1 bg-black/70 backdrop-blur-sm rounded-full px-1 py-0.5">
              <span className="text-white text-[7px] xs:text-[9px]">{series.seasons} {t('series.seasons')}</span>
            </div>
          )}
          <div className="absolute bottom-1 left-1 bg-purple-600/80 backdrop-blur-sm rounded-full px-1 py-0.5 hidden sm:block">
            <span className="text-white text-[6px] xs:text-[8px]">🎨 رسوم متحركة</span>
        ...
        <div className="mt-0.5 xs:mt-1">
          <h3 className="text-white font-semibold text-[9px] xs:text-[11px] sm:text-[12px] leading-tight line-clamp-1 group-hover:text-red-500 transition">{series.title}</h3>
          <div className="flex items-center gap-0.5 text-gray-400 text-[7px] xs:text-[9px] mt-0.5">
            <FaCalendarAlt className="text-[6px] xs:text-[8px]" />
            <span>{series.year}</span>
            <span className="w-0.5 h-0.5 bg-gray-600 rounded-full"></span>
            <span className="line-clamp-1 text-[6px] xs:text-[8px]">{series.genre}</span>
          </div>
        </div>
      </Link>
      <button onClick={(e) => onToggleWatchlist(series.id, itemType, e)} className={`absolute bottom-14 right-1 p-1 rounded-full transition ${isInWatchlist ? 'bg-purple-600 text-white' : 'bg-gray-800/80 text-gray-400 hover:bg-gray-700'}`} title={isInWatchlist ? 'في قائمتي' : 'إضافة إلى قائمتي'}>
        <FaBookmark size={10} />
      </button>
    </div>
  );
...
const MovieListItem = ({ movie, onToggleWatchlist, isInWatchlist, itemType }) => {
  return (
    <div className="block group relative">
      ...
      <button onClick={(e) => onToggleWatchlist(movie.id, itemType, e)} className={`absolute top-3 right-3 p-1.5 rounded-full transition ${isInWatchlist ? 'bg-purple-600 text-white' : 'bg-gray-800/80 text-gray-400 hover:bg-gray-700'}`} title={isInWatchlist ? 'في قائمتي' : 'إضافة إلى قائمتي'}>
        <FaBookmark size={12} />
      </button>
    </div>
  );
...
const SeriesListItem = ({ series, onToggleWatchlist, isInWatchlist, itemType }) => {
  ...
  return (
    <div className="block group relative">
      ...
      <button onClick={(e) => onToggleWatchlist(series.id, itemType, e)} className={`absolute top-3 right-3 p-1.5 rounded-full transition ${isInWatchlist ? 'bg-purple-600 text-white' : 'bg-gray-800/80 text-gray-400 hover:bg-gray-700'}`} title={isInWatchlist ? 'في قائمتي' : 'إضافة إلى قائمتي'}>
        <FaBookmark size={12} />
      </button>
    </div>
  );
