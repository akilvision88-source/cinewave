// src/pages/movies/EuropeanMovies.js
...
const EuropeanMovies = () => {
  ...
  const [viewMode, setViewMode] = useState(() => {
    const savedMode = localStorage.getItem('european_view_mode');
    return savedMode === 'list' ? 'list' : 'grid';
  });
  const [currentPage, setCurrentPage] = useState(() => {
    const savedPage = localStorage.getItem('european_current_page');
  ...
  const genres = ['all', 'Drame', 'Comédie', 'Crime', 'Thriller', 'Action', 'Romance', 'Historique'];
  const years = ['all', '2024', '2023', '2022', '2021', '2020', '2019', '2018', '2017', '2016', '2015'];

  const handleViewModeChange = (mode) => {
    setViewMode(mode);
    localStorage.setItem('european_view_mode', mode);
  };

  const handlePageChange = (page) => {
    setCurrentPage(page);
    localStorage.setItem('european_current_page', page);
  };
  ...
  // تحميل أفلام أوروبية فقط
  useEffect(() => {
    const loadMovies = async () => {
      setLoading(true);
      try {
        const data = await moviesAPI.getByCategory('european');
        setMovies(data);
        setFilteredMovies(data);
      } catch (error) {
        console.error('خطأ في تحميل الأفلام الأوروبية:', error);
        setMovies([]);
        setFilteredMovies([]);
      } finally {
        setLoading(false);
  ...
  return (
    <div className="min-h-screen bg-black">
      ...
      <div className="relative h-[20vh] md:h-[25vh] w-full overflow-hidden">
        ...
        <div className="absolute inset-0 bg-cover bg-center" style={{ backgroundImage: 'url(https://image.tmdb.org/t/p/original/gEU2QniE6E77NI6lCU6MxlNBvIx.jpg)', filter: 'brightness(0.4)' }} />
        <div className="relative h-full w-full px-3 sm:px-4 md:px-6 lg:px-8 flex items-center z-20">
          <div>
            <div className="flex items-center gap-2 mb-1">
              <span className="text-2xl">🇪🇺</span>
              <h1 className="text-2xl md:text-3xl font-bold text-white">أفلام أوروبية</h1>
            </div>
            <p className="text-gray-300 text-sm md:text-base max-w-2xl">أفضل الأفلام الأوروبية</p>
          </div>
        </div>
      </div>
      ...
      <div className="w-full px-3 sm:px-4 md:px-6 lg:px-8 py-4">
        ...
        {filteredMovies.length === 0 ? (
          <div className="text-center py-12">
            ...
            <p className="text-gray-500 text-lg">لا توجد أفلام أوروبية حالياً</p>
