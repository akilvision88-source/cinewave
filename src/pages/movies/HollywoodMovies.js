// src/pages/movies/HollywoodMovies.js
...
const HollywoodMovies = () => {
  ...
  const [viewMode, setViewMode] = useState(() => {
    const savedMode = localStorage.getItem('hollywood_view_mode');
    return savedMode === 'list' ? 'list' : 'grid';
  });
  const [currentPage, setCurrentPage] = useState(() => {
    const savedPage = localStorage.getItem('hollywood_current_page');
  ...
  const genres = ['all', 'Action', 'Drame', 'Comédie', 'Science-Fiction', 'Thriller', 'Horreur', 'Romance', 'Crime', 'Fantastique'];
  const years = ['all', '2024', '2023', '2022', '2021', '2020', '2019', '2018', '2017', '2016', '2015', '2014', '2013', '2012', '2011', '2010'];

  const handleViewModeChange = (mode) => {
    setViewMode(mode);
    localStorage.setItem('hollywood_view_mode', mode);
  };

  const handlePageChange = (page) => {
    setCurrentPage(page);
    localStorage.setItem('hollywood_current_page', page);
  };
  ...
  // تحميل أفلام هوليود فقط
  useEffect(() => {
    const loadMovies = async () => {
      setLoading(true);
      try {
        const data = await moviesAPI.getByCategory('hollywood');
        setMovies(data);
        setFilteredMovies(data);
      } catch (error) {
        console.error('خطأ في تحميل أفلام هوليود:', error);
        setMovies([]);
        setFilteredMovies([]);
      } finally {
        setLoading(false);
        ...
        <div className="relative h-full w-full px-3 sm:px-4 md:px-6 lg:px-8 flex items-center z-20">
          <div>
            <div className="flex items-center gap-2 mb-1">
              ...
              <h1 className="text-2xl md:text-3xl font-bold text-white">أفلام هوليود</h1>
            </div>
            <p className="text-gray-300 text-sm md:text-base max-w-2xl">أكبر نجاحات هوليود</p>
          </div>
        </div>
      </div>
      ...
      <div className="w-full px-3 sm:px-4 md:px-6 lg:px-8 py-4">
        ...
        {filteredMovies.length === 0 ? (
          <div className="text-center py-12">
            ...
            <p className="text-gray-500 text-lg">لا توجد أفلام هوليود حالياً</p>
