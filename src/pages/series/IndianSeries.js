...
const IndianSeries = () => {
  ...
  const [viewMode, setViewMode] = useState(() => {
    const savedMode = localStorage.getItem('indianseries_view_mode');
    return savedMode === 'list' ? 'list' : 'grid';
  });
  const [currentPage, setCurrentPage] = useState(() => {
    const savedPage = localStorage.getItem('indianseries_current_page');
    return savedPage ? parseInt(savedPage) : 1;
  });
  const [itemsPerPage] = useState(30);
  const [isMobile, setIsMobile] = useState(window.innerWidth < 768);

  const genres = ['all', 'Drame', 'Romance', 'Comédie', 'Action', 'Thriller', 'Historique'];
  const years = ['all', '2024', '2023', '2022', '2021', '2020', '2019', '2018', '2017', '2016', '2015', '2014', '2013', '2012', '2011', '2010'];

  const handleViewModeChange = (mode) => {
    setViewMode(mode);
    localStorage.setItem('indianseries_view_mode', mode);
  };

  const handlePageChange = (page) => {
    setCurrentPage(page);
    localStorage.setItem('indianseries_current_page', page);
  };
  ...
  useEffect(() => {
    const loadSeries = async () => {
      setLoading(true);
      try {
        const data = await seriesAPI.getByCategory('indian');
        setSeries(data);
    ...
    <div className="min-h-screen bg-black">
      <div className="relative h-[20vh] md:h-[25vh] overflow-hidden">
        ...
        <div className="relative h-full container-custom flex items-center z-20">
          <div>
            <h1 className="text-2xl md:text-3xl font-bold text-white">مسلسلات هندية</h1>
            <p className="text-gray-300 text-sm md:text-base max-w-2xl mt-1">{t('series.indianSeriesDescription')}</p>
          </div>
        </div>
      </div>
