...
const TurkishSeries = () => {
  ...
  const [viewMode, setViewMode] = useState(() => {
    const savedMode = localStorage.getItem('turkishseries_view_mode');
  ...
  const handleViewModeChange = (mode) => {
    setViewMode(mode);
    localStorage.setItem('turkishseries_view_mode', mode);
  };

  const handlePageChange = (page) => {
    setCurrentPage(page);
    localStorage.setItem('turkishseries_current_page', page);
  };
  ...
  useEffect(() => {
    const loadSeries = async () => {
      setLoading(true);
      try {
        const data = await seriesAPI.getByCategory('turkish');
        setSeries(data);
    ...
    <div className="min-h-screen bg-black">
      <div className="relative h-[20vh] md:h-[25vh] overflow-hidden">
        ...
        <div className="relative h-full container-custom flex items-center z-20">
          <div>
            <h1 className="text-2xl md:text-3xl font-bold text-white">مسلسلات تركية</h1>
            <p className="text-gray-300 text-sm md:text-base max-w-2xl mt-1">{t('series.turkishSeriesDescription')}</p>
          </div>
        </div>
      </div>
