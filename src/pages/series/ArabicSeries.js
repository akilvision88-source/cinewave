...
const ArabicSeries = () => {
  ...
  const [viewMode, setViewMode] = useState(() => {
    const savedMode = localStorage.getItem('arabicseries_view_mode');
    return savedMode === 'list' ? 'list' : 'grid';
  });
  const [currentPage, setCurrentPage] = useState(() => {
    const savedPage = localStorage.getItem('arabicseries_current_page');
    return savedPage ? parseInt(savedPage) : 1;
  });
  const [itemsPerPage] = useState(30);
  const [isMobile, setIsMobile] = useState(window.innerWidth < 768);

  const genres = ['all', 'Drame', 'Comédie', 'Action', 'Romance', 'Historique', 'Thriller'];
  const years = ['all', '2024', '2023', '2022', '2021', '2020', '2019', '2018', '2017', '2016', '2015'];

  const handleViewModeChange = (mode) => {
    setViewMode(mode);
    localStorage.setItem('arabicseries_view_mode', mode);
  };

  const handlePageChange = (page) => {
    setCurrentPage(page);
    localStorage.setItem('arabicseries_current_page', page);
  };
  ...
  useEffect(() => {
    const loadSeries = async () => {
      setLoading(true);
      try {
        const data = await seriesAPI.getByCategory('arabic');
        setSeries(data);
    ...
    <div className="min-h-screen bg-black">
      <div className="relative h-[20vh] md:h-[25vh] overflow-hidden">
        ...
        <div className="relative h-full container-custom flex items-center z-20">
          <div>
            <h1 className="text-2xl md:text-3xl font-bold text-white">مسلسلات عربية</h1>
            <p className="text-gray-300 text-sm md:text-base max-w-2xl mt-1">{t('series.arabicSeriesDescription')}</p>
          </div>
        </div>
      </div>
