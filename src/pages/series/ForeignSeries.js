...
const ForeignSeries = () => {
  ...
  const [viewMode, setViewMode] = useState(() => {
    const savedMode = localStorage.getItem('foreignseries_view_mode');
    return savedMode === 'list' ? 'list' : 'grid';
  });
  const [currentPage, setCurrentPage] = useState(() => {
    const savedPage = localStorage.getItem('foreignseries_current_page');
    return savedPage ? parseInt(savedPage) : 1;
  });
  const [itemsPerPage] = useState(30);
  const [isMobile, setIsMobile] = useState(window.innerWidth < 768);

  const genres = ['all', 'Drame', 'Action', 'Science-Fiction', 'Comédie', 'Crime', 'Fantastique', 'Thriller', 'Horreur'];
  const years = ['all', '2024', '2023', '2022', '2021', '2020', '2019', '2018', '2017', '2016', '2015'];

  const handleViewModeChange = (mode) => {
    setViewMode(mode);
    localStorage.setItem('foreignseries_view_mode', mode);
  };

  const handlePageChange = (page) => {
    setCurrentPage(page);
    localStorage.setItem('foreignseries_current_page', page);
  };
  ...
  useEffect(() => {
    const loadSeries = async () => {
      setLoading(true);
      try {
        const data = await seriesAPI.getByCategory('foreign');
        setSeries(data);
  ...
  useEffect(() => {
    let results = [...series];
    if (searchTerm) results = results.filter(s => s.title.toLowerCase().includes(searchTerm.toLowerCase()));
    ...
    <div className="min-h-screen bg-black">
      <div className="relative h-[20vh] md:h-[25vh] overflow-hidden">
        ...
        <div className="relative h-full container-custom flex items-center z-20">
          <div>
            <h1 className="text-2xl md:text-3xl font-bold text-white">مسلسلات أجنبية</h1>
            <p className="text-gray-300 text-sm md:text-base max-w-2xl mt-1">{t('series.foreignSeriesDescription')}</p>
          </div>
        </div>
      </div>
      ...
      <div className="container-custom py-4">
        ...
        {filteredSeries.length === 0 ? (
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-3">
            {currentSeries.map((serie) => <SeriesCard key={serie.id} serie={serie} />)}
          </div>
        ) : (
          <div className="space-y-2">
            {currentSeries.map((serie) => <SeriesListItem key={serie.id} serie={serie} />)}
          </div>
        )}
    ...
    <Link to={`/series/${serie.id}`} className="block group">
      <div className="flex gap-3 bg-gray-900/50 rounded-lg p-2 hover:bg-gray-800 transition border border-gray-800 hover:border-red-500/50">
        ...
        <div className="flex-1">
          ...
          <div className="flex flex-wrap items-center gap-2 text-xs text-gray-400 mt-1"><span className="flex items-center gap-0.5"><FaStar className="text-yellow-400 text-[10px]" /> {serie.rating || '?'}/10</span><span className="flex items-center gap-0.5"><FaCalendarAlt className="text-[10px]" /> {serie.year}</span><span>{serie.genre}</span></div>
